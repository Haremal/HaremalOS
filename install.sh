...
read -p "Install Jetbrains Toolbox? (y/n) " I_TOOLBOX
...
[[ "$EFI_CONFIRM" =~ [Yy] ]] && mkfs.fat -F 32 "$EFI_P"
mkfs.ext4 -F "$ROOT_P"
...
# 10. We must pass the Dual Boot status into the chroot via a file
